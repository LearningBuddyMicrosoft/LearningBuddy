...
from api_client import get_attempt_report
...
if st.button("📄 Generate & Download PDF Report", use_container_width=True):
    with st.spinner("Preparing your report..."):
        pdf_content = get_attempt_report(attempt_id)
        if pdf_content:
            st.download_button(
                label="📥 Click here to Download",
                data=pdf_content,
                file_name=f"LearningBuddy_Report_{attempt_id}.pdf",
                mime="application/pdf"
            )
        else:
            st.error("Could not retrieve report. Please try again.")
